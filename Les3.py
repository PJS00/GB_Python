...
# def sumNumbers(n, y = 'hello'): #сколько аргументов мы передаем,
# # столько и принимаем, здесь задаем y по умолчанию
# # Если мы не передаем еще один аргумент (только n -> 5),
# # то выводится то, что задали по умолчанию y = 'hello'
# # Если передаем два аргумента, то выводится два аргумента
# # n -> 5, y -> 'qwerty'
#     print(y)
#     summa = 0
#     for i in range(1, n + 1):
...
# Функция, которая принимает неогранченное количество аргументов
# Например передаем множество букв, ходим получить целое слово
# def sum_str(*args):
#     res = ''
#     for i in args:
#         res += i
...
# Модуль - файлик, куда записываем все фенкции, откуда вызываем их из другого файла

# это должно быть в другом файле modul.py
## def max1(a, b):
##     if a > b:
##         return a # функция return прерывает работу программы
## # если у нас прервется здесь, то зачем нам писать еще какое-то условие?
##     else:
##         return b
## # запишем по-другому
## def max1(a, b):
...
# чтобы получить этуфункцию из файла modul.py, пишем
# import modul1 # указываем имя фала, откуда импортируем
# обращаемся к нашему файлу и к функции, которая там записана
# print(modul1.max1(5, 9)) # 9

# from modul1 import max1 #напрямую импортируем функцию
# print(max1(5, 9)) # 9

# если мы не знаем, как назвается функция, чтобы не перечислять все названия,
#можно записать
# from modul1 import * # он импортирует все функции
# print(max1(5, 9)) # 9

# можно в данном файле переименовать файл-модуль для удобства работы 
# import modul1 as m1
# print(m1.max1(5, 9)) # 9
...
# Alias (псевдоним) — альтернативное имя, которое даётся функции при еt
# импорте из файла.
...
# Петр: Число больше 50? Иван: Да. Значит число от 50 до 100. Делим пополам
# Петр: Число больше 75? Иван: Да. Значит число от 75 до 100 Делим пополам
# Петр: Число больше 87? Иван: Нет. Значит число от 75 до 87 Делим пополам
# Петр: Число больше 81? Иван: Нет. Значит число от 75 до 81 Делим пополам
# Петр: Число больше 78? Иван: Нет. Значит число от 75 до 78 Делим пополам
...
# def merge_sort(nums):
#     if len(nums) > 1: # указываем не базис, а случай, при котором наша программа будет выполняться
#         mid = len(nums) // 2 # поделили список пополам, середина
#         left = nums[:mid] # 1 список из начала
#         right = nums[mid:] # 2 список из конца
#         merge_sort(left) # вызываем рекурсию, делим список снова пополам
#         merge_sort(right)
#         i = j = k = 0
